...
date: "Tue 25th and Wed 26th Augsut 2026"
---
...
*A video recording of me working through practical 1 in GeoDa is available on [YouTube](https://youtu.be/huPDY9W62EU).*
...
- Browse to and open **Greater_Manchester_LSOA_Age_IMD.gpkg** 
- Select **OK** to open the layer called **Greater_Manchester_LSOA_Age_IMD**. 

![](images/geoda-file-open-gpkg.png){ height=30% }


GeoDa should open a map display of the data automatically. We can create a simple choropleth map in here by right clicking on the map and then select **Change Current Map Type**. Select **Natural Breaks > 5** and choose the variable to map (we want **imddecile**). Then a classified map will appear, similar to one we have created in RStudio. 

Also open the attribute table ![Attribute Table](images/attribute-table.png){ height=30 } and a histogram ![Histogram](images/histogram.png){ height=30 } for `imddecile`. GeoDa allows us to use brushing or linked displays. Select one of the larger bars in the histogram and you will see that the same entries are highlighted on the map and in the attribute table. You can also draw a box on the map (left-click and drag) and that will highlight the entries as well (you can drag the box around). 
...
- Select **Add ID Variable...** and click **Add Variable** to accept the default name.  
- You now need to choose the neighbourhood weight method.  Select **Queen contiguity** and click **Create**.  
- The neighbourhood weight matrix is saved in a file.  A suggested name is given.  This is the LSOA shape file name with a suffix of .gal.  Select **Save** and after a few seconds you should see a message **Weights file `Greater_Manchester_LSOA_Age_IMD.gal` created successfully**.  
...
Click ![](images/histogram-button.png){ height=30 } within the Weights Manager window and you can see how many neighbours each polygon has. In my data, five is the most common number of neighbours. Close this and open the **Connectivity Map**, which shows the neighbours for each polygon. Close this and the Weights Manager when you are happy with it. 

## Moran's I

We can now start to explore the spatial patterns of the model we want to develop.  Click on the **Space** menu on the GeoDa window.  First we are going to look at the global indicator of spatial autocorrelation.  Select the first menu item **Univariate Moran's I**.  A list of variables is displayed.  Choose the `imdrank` variable.  Select the weights file we created earlier. 

![](images/morans-i.png){ height=40% }

A new window opens with a scatter plot of the imdrank variable and the lagged imdrank variable.  The lagged imdrank variable is the value of the IMD Rank for the selected neighbourhood and its neighbours as defined when we set the spatial weights.  The scatter-plot suggests a positive correlation between the variables.  This means that there is a weak positive spatial autocorrelation. It has a value of 0.565 confirming this positive spatial autocorrelation. We will now look at the local indicators of spatial association.  

## Local Moran's I / Local Indicators of Spatial Association (LISA)

- Select **Space > Univariate Local Moran's I**. 
- Select the `imdrank` variable from the list displayed and select the weights file used earlier (it should be selected automatically). 
...
![](images/lisa-high-low.png){ height=35% }

![](images/lisa-sig.png){ height=35% }

The first map shows the extent to which neighbouring areas are clustered:  
  
- darker blue areas are deprived neighbourhoods (ranks 21, 24, 28,etc.) surrounded by similarly deprived neighbourhoods 
- red areas are well-off neighbourhoods (ranks 33688, 33744, 33750, etc.) surrounded by similarly well-off neighbourhoods 
- pink areas are well-off neighbourhoods surrounded by deprived neighbourhoods 
- lighter blue areas are deprived neighbourhoods surrounded by well-off neighbourhoods 
...
*Remember the ranks are for the whole country, out of 33,755 LSOAs across England. If you want to order the data by the `imdrank` column in the attribute table view, double click on the column header*. 
...
Load the `Greater_Manchester_LSOA_Age_IMD.gpkg` file from the `trams.zip` file:
...
`lag` is now our lagged value (Y axis). `manchester_lsoa$IMDrank` is our IMD rank (X axis). 
...
We can also perform our LISA calculation. We need to extract the column we want to use (`IMDrank`) and then use that with the existing weights variable:
...
*There are 2 measures in this data - the IMD rank and decile. We will primarily be using the decile, where the area in decile 1 is the most deprived.*
...
It's worth mentioning we are using `tm_scale_discrete()` rather than `tm_scale_intervals` (which is what we have used before in the Introduction course) because we are mapping deciles (integers 1 through 10) rather than values we want to group. See https://tmap.geocompx.org/scales for more information. 
...
It's worth noting that we can plot maps with multiple layers. With `tmap` each layer needs to have a `tm_shape()` function first and then a second `tm_*` function. Which function we use depends on what type of data we are working with (i.e. points, lines or polygons) and how we want to show it (e.g. fill or outline). We often use `tm_polygons()` to fill polygons, and above we have used `tm_borders()` to show just borders (i.e. no fill) and `tm_sybmols()` to show points. 
...
We have just created a new data frame which links the tram_stations with the LSOAs. Most LSOAs only have one tram station in them, but a few LSOAs have more than one tram station in them. 

We can see this by counting how many tram stations are in each LSOA: 
...
First of all we need to know which LSOA we are interested in. R uses indexes to identify things. So we know that Manchester 054C has more than one station in it, and we can use `which()` to find out the index of Manchester 054C.

```{r, comment=NA,warning=FALSE,message=FALSE,error=FALSE}
which(manchester_lsoa$name == "Manchester 054C")
```
Which will tell us it is `1610`. We can then plug this into some map code:
...
Here we see the tram stations (red dots) the tram line (black) and the LSOA boundaries (grey). *Remember the order of the map layers is key - the first one (in this case `tm_shape(manchester_lsoa[1610,])`) sets what is shown on the map, and then the rest are added in order, first to last.* 
...
*You may have spotted that I used three IDs at the beginning of the map code: `c(84,19,94)`. Tmap sets the map area based on the first layer that is plotted. If we had just plotted St Werburgh's Road `c(84)` then the map would be centered on this tram station and not show the surrounding areas. So I added in two nearby stations (`19` and `94`) to show a larger area. Try it with only one station and see what it looks like!*

*How did I find out which IDs to use? I actually loaded the data in QGIS and used Identify Features to click on the nearby stations to find the ones I needed. However I could also use the `tmap_mode("view")` option to see the interactive map, and use that to click on the stations.*
...
```{r, comment = NA}
#group by Station, take average IMDdecile. 
...
The `%>%` command (known as a pipe) is a piece of code from the tidyverse. It allows you to pass the output from one function in to another function. This is sometimes known as chaining commands together. Here, we take `tram_stations_IMD`, pass it to `group_by(RSTNAM)` and then pass on to `summarise(mean(IMDdecile))`. 

Now we have each station buffer listed once, with the average IMD decile (`mean(IMDdecile)`). Currently they are still the buffers, which won't make sense to map, so we can convert them back to points (centroids):
...
- We calculated the average (`mean(IMDrank)`) IMD rank for each tram station catchment area. Are there better measures for quantifying who has access, given we are interested in looking at access for the most deprived areas. Would `max(IMDrank)`, to show the most deprived LSOA, be more useful? Can you update the code for this?
...
We can extend this to IMD score, with the assumption if 68% of one deprived area is within the tram station catchment area, but 100% of a different area is within the catchment area of a different tram station, it is more important to prioritise the deprived area with 100% coverage than the one with 68% coverage. Therefore a 100% weight is given to one, and only a 68% weight is given to another. We can calculate the weights by measuring the area of the LSOA within the buffer (e.g 70% of the LSOA, which is 80m^2, and then if 40% of a different LSOA is in the buffer, this area would be 38m^2). We then total up all the areas and work out the proportion of the buffer that that area is. 
...
This practical was written using GeoDa 1.22.0.21 (25 September 2025), R 4.5.3 (2026-03-11), RStudio 2026.01.1+403 and `tmap` 4.2, last updated on 15/07/2026 by Dr. Nick Bearman (nick@nickbearman.com). It has also been tested in Posit Cloud, with R 4.6.0 (2026-04-24), RStudio 2026.01.1+403 and `tmap` 4.3. 
...
The latest version of the PDF is available from https://github.com/nickbearman/advanced-r-gis. This is v7.2 and this was created on `r format(Sys.time(), '%d %B %Y')`.
