...
Calculate Moran's I and LISA in R | `rgeoda()` (pg. 5)
Perform a spatial join | `st_join()` (pg. 13)
Know how to reorder data | `order()` (pg. 15) 
Know how to use buffers within R  | `st_buffer()` (pg. 16)   
...
We can replicate the Moran's I scatter plot in R. As far as I can tell, this is not built into the RGeoDa library, so there is a bit of leg work to replicate it. *If you are interested in the background, have a look at https://github.com/GeoDaCenter/rgeoda/issues/41*. 
...
We also need to calculate the Moran's I value. This is the correlation between the IMD data (`imd`) and the lagged data (`lag`) multiplied by the standard diviation of the lagged data divided by the standard diviation of the imd data. 
...
*The next version of the IMD for England is due out in 2025 or 2026 and usually Wales, Scotland and NI follow soon after that. More details at https://ocsi.uk/2023/07/10/we-are-updating-the-english-indices-of-deprivation/ if you are interested.*
...
Here we see the stations (red dots) the line (black) and the LSOA boundaries (grey). *Remember the order of the map layers is key - the first one (in this case `tm_shape(manchester_lsoa[1643,])`) sets what is shown on the map, and then the rest are added in order, first to last.* 
...
The latest version of the PDF is available from https://github.com/nickbearman/confident-spatial-analysis. This is v7.0 and this was created on `r format(Sys.time(), '%d %B %Y')`.
