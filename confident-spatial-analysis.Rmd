...
Understand how to read in a variety of formats | `readOGR()` (pg. 5)  
Know how to reorder data | `order()` (pg. 12) 
...
We also have a version of the code above in `sf`. We need the `tidyverse` library for some of the analysis too. 
...
stations_in_LSOA <- st_join(tram_stations, manchester_lsoa, join = st_within)
...
st_join(tram_stations, manchester_lsoa) %>% View()

joined <- st_join(tram_stations, manchester_lsoa)

joined[1,]
class(joined)
qtm(joined)

joined <- joined[order(joined$IMDscore, decreasing = TRUE), ]

View(joined)

#stations
tm_shape(joined) +
  tm_dots(joined, size = 0.1, shape = 19, col = "darkred") 

tm_shape(joined) +
  tm_dots(joined, size = 0.1, shape = 19, col = "darkred") +
  tm_shape(joined[1:10,]) +
  tm_dots(joined[1:10,], size = 0.1, shape = 19, col = "red") +
  tm_shape(joined[82:92,]) +
  tm_dots(joined[82:92,], size = 0.1, shape = 19, col = "blue") 
...
#still buffers, so convert to points (centriods)
...
This practical was written using R 3.5.1 (2018-07-02) and RStudio 1.1.463 by Dr. Nick Bearman (nick@geospatialtrainingsolutions.co.uk). It has also been tested in R 4.1.2 (2021-11-01) and RStudio 2021.09.2 (RStudio Cloud).
