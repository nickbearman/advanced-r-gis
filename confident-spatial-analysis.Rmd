...
Read the CSV file in (`TfGMMetroRailStops.csvv`) and it should look like this:
...
Now we know where the tram stations are, and we can overlay that with the LSOA data. 
...
station_LSOA_IMD_pt_ordered <- station_LSOA_IMD_pt[order(station_LSOA_IMD_pt$`mean(IMDscore)`, 
            decreasing = TRUE), ]
...
Secondly, load the tramline data, available as a GeoJSON file. This is a different type of file to a shape file and is an XML based format (open it in Notepad if you are interested in how it is structured). `st_read()` can read all sorts of file formats, so we can adapt the code easily.   
